...
def main():
    ...
    # 初始化需要对比的三个模型（参数设置参照 test_probabilistic）
    models = {
        ...
        "Advanced Probabilistic RAC (ratio=0.3)": RacAdvancedProbabilisticClassifier(kernel_type='rbf', kernel_pars=[10.7], c=7.5, uncertainty_ratio=0.3)
    }
    ...
    output_path = os.path.join(os.path.dirname(__file__), 'compare_models_results.xlsx')
    
    df_results.to_excel(output_path, index=False)
    print(f"\n[结果] 模型对比指标已保存至 {output_path}")
